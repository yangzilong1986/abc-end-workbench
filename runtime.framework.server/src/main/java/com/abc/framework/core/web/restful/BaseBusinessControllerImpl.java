package com.abc.framework.core.web.restful;
...
import com.abc.framework.core.api.BusinessController;
...
import com.abc.framework.core.web.domain.ControllerPath;
import com.abc.framework.core.web.domain.ErrorDescripter;
	...
	implements BusinessController<T, Q> 
{
	...
	public Result selectList(@RequestBody Q query) {
		...
		if (query==null) {
			...
			return new Result(Status.ADDCHECK, errorDescripter.getControllerSelectListArgumentInfo());
		}
		boolean isEmpty=BeanUtils.isEmpty(query);
		if(isEmpty){
			Result result=new Result(Status.ADDCHECK, errorDescripter.getControllerSelectListArgumentInfo());
			result.addError(Status.ADDCHECK, errorDescripter.getControllerSelectListArgumentInfo());
			return result;
		}
		List<T> list= null;
		try{
			...
			log.error("添加对象失败:" + path.getEntityName(), e);
			Result result=new  Result(Status.ADDERROR, errorDescripter.getControllerAddOneErrorInfo());
			result.addError(Status.ADDERROR, e.getMessage());
			return result;
