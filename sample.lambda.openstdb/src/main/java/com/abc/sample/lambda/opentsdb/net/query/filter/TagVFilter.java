...
package com.abc.sample.lambda.opentsdb.net.query.filter;
...
public abstract class TagVFilter implements Comparable<TagVFilter> {
  ...
  public Deferred<byte[]> resolveTagkName(final TSDB tsdb) {
    ...
    return tsdb.getUIDAsync(UniqueId.UniqueIdType.TAGK, tagk)
  ...
  public Deferred<byte[]> resolveTags(final TSDB tsdb, 
    ...
    for (final String tagv : literals) {
      tagvs.add(tsdb.getUIDAsync(UniqueId.UniqueIdType.TAGV, tagv)
    ...
    tagvs.add(tsdb.getUIDAsync(UniqueId.UniqueIdType.TAGK, tagk)
