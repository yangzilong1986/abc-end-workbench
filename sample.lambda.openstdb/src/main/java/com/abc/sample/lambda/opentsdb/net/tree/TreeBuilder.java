...
package com.abc.sample.lambda.opentsdb.net.tree;
...
public final class TreeBuilder {
  ...
  private boolean processRuleset(final Branch parent_branch, int depth) {
    ...
    for (Map.Entry<Integer, TreeRule> entry : rule_level.entrySet()) {
      ...
      if (rule.getType() == TreeRule.TreeRuleType.METRIC) {
        parseMetricRule();
        // local_branch = current_branch; //do we need this???
      } else if (rule.getType() == TreeRule.TreeRuleType.TAGK) {
        parseTagkRule();
      } else if (rule.getType() == TreeRule.TreeRuleType.METRIC_CUSTOM) {
        parseMetricCustomRule();
      } else if (rule.getType() == TreeRule.TreeRuleType.TAGK_CUSTOM) {
        parseTagkCustomRule();
      } else if (rule.getType() == TreeRule.TreeRuleType.TAGV_CUSTOM) {
        parseTagvRule();
      } else {
  ...
  private void parseTagkRule() {
    ...
    for (UIDMeta uidmeta : tags) {
      if (uidmeta.getType() == UniqueId.UniqueIdType.TAGK &&
          uidmeta.getName().equals(rule.getField())) {
        found = true;
      } else if (uidmeta.getType() == UniqueId.UniqueIdType.TAGV && found) {
  ...
  private void parseTagkCustomRule() {
    ...
    for (UIDMeta tag: meta.getTags()) {
      if (tag.getType() == UniqueId.UniqueIdType.TAGK &&
  ...
  private void parseTagvRule() {
    ...
    UIDMeta tagv = null;
    for (UIDMeta tag: meta.getTags()) {
      if (tag.getType() == UniqueId.UniqueIdType.TAGV &&
          tag.getName().equals(rule.getField())) {
        tagv = tag;
        break;
      }
  ...
  private void setCurrentName(final String original_value, 
    ...
    if (format.contains("{tag_name}")) {
      final TreeRule.TreeRuleType type = rule.getType();
      if (type == TreeRule.TreeRuleType.TAGK) {
        format = format.replace("{tag_name}", rule.getField());
      } else if (type == TreeRule.TreeRuleType.METRIC_CUSTOM ||
          type == TreeRule.TreeRuleType.TAGK_CUSTOM ||
          type == TreeRule.TreeRuleType.TAGV_CUSTOM) {
