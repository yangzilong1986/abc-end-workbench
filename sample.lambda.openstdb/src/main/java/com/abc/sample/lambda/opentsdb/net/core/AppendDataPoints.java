...
public class AppendDataPoints {
   ...
   */
  public final Collection<Internal.Cell> parseKeyValue(final TSDB tsdb, final KeyValue kv) {
    ...
    final Map<Integer, Internal.Cell> deltas = new TreeMap<Integer, Internal.Cell>();
    ...
    try {
      while (val_idx < kv.value().length) {
        ...
        final Internal.Cell duplicate = deltas.get(delta);
        if (duplicate != null) {
    ...
    for (final Internal.Cell cell: deltas.values()) {
