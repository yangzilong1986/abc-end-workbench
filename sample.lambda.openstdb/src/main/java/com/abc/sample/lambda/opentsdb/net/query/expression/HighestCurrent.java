...
package com.abc.sample.lambda.opentsdb.net.query.expression;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

import com.abc.sample.lambda.opentsdb.net.core.AggregationIterator;
import com.abc.sample.lambda.opentsdb.net.core.Aggregator;
import com.abc.sample.lambda.opentsdb.net.core.Aggregators;
import com.abc.sample.lambda.opentsdb.net.core.DataPoint;
import com.abc.sample.lambda.opentsdb.net.core.DataPoints;
import com.abc.sample.lambda.opentsdb.net.core.MutableDataPoint;
import com.abc.sample.lambda.opentsdb.net.core.SeekableView;
...
public class HighestCurrent implements Expression {
  ...
  public DataPoints[] evaluate(final TSQuery data_query,
    ...
    final HighestMax.TopNSortingEntry[] max_by_ts =
        new HighestMax.TopNSortingEntry[num_results];
    if (aggregator.hasDoubles() && aggregator.hasLongs()) {
      for (int i = 0; i < num_results; i++) {
        max_by_ts[i] = new HighestMax.TopNSortingEntry(
            Math.max((double)max_longs[i], max_doubles[i]), i);
      }
    } else if (aggregator.hasLongs() && !aggregator.hasDoubles()) {
      for (int i = 0; i < num_results; i++) {
        max_by_ts[i] = new HighestMax.TopNSortingEntry((double) max_longs[i], i);
      }
    } else if (aggregator.hasDoubles() && !aggregator.hasLongs()) {
      for (int i = 0; i < num_results; i++) {
        max_by_ts[i] = new HighestMax.TopNSortingEntry(max_doubles[i], i);
      }
    }
  ...
  public static class MaxLatestAggregator extends Aggregator {
     ...
     */
    public MaxLatestAggregator(final Aggregators.Interpolation method, final String name,
