...
package com.abc.sample.lambda.opentsdb.net.meta;
...
import com.abc.sample.lambda.opentsdb.net.utils.JSONException;
...
public final class TSMeta {
  ...
  public Deferred<Boolean> syncToStorage(final TSDB tsdb, 
    ...
    uid_group.add(tsdb.getUidName(UniqueId.UniqueIdType.METRIC, metric_uid)
        .addCallback(new UidCB()));
    
    int idx = 0;
    for (byte[] tag : parsed_tags) {
      if (idx % 2 == 0) {
        uid_group.add(tsdb.getUidName(UniqueId.UniqueIdType.TAGK, tag)
            .addCallback(new UidCB()));
      } else {
        uid_group.add(tsdb.getUidName(UniqueId.UniqueIdType.TAGV, tag)
            .addCallback(new UidCB()));
      }
      idx++;
    }
  ...
  private static class LoadUIDs implements Callback<Deferred<TSMeta>, TSMeta> {
    ...
    public Deferred<TSMeta> call(final TSMeta meta) throws Exception {
      ...
      uid_group.add(UIDMeta.getUIDMeta(tsdb, UniqueId.UniqueIdType.METRIC,
      ...
      for (byte[] tag : tags) {
        if (idx % 2 == 0) {
          uid_group.add(UIDMeta.getUIDMeta(tsdb, UniqueId.UniqueIdType.TAGK, tag)
                .addCallback(new UIDMetaCB(idx)));
        } else {
          uid_group.add(UIDMeta.getUIDMeta(tsdb, UniqueId.UniqueIdType.TAGV, tag)
              .addCallback(new UIDMetaCB(idx)));
        }          
        idx++;
      }
