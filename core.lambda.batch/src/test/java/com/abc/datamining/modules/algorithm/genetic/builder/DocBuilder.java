package com.abc.datamining.modules.algorithm.genetic.builder;
...
public class DocBuilder {
	...
	private Logger logger = Logger.getLogger(DocBuilder.class);
	...
	private DocumentSet loadDocuments() {
		...
		try {
			...
			for (int i = 1; i < words_len; i++) {
				String word = words[i];
				if ("月".endsWith(word) || "日".endsWith(word) || "点".endsWith(word)) {
 			...
 			for (int i = 0; i < 50; i++) {
 				...
 				doc.setName("doc_" + i);
 				String[] take_words = new String[take_len];
 				for (int j = 0; j < take_len; j++) {
 					take_words[j] = words[RandomUtils.nextInt(words_len)];
 				}
 				doc.setWords(take_words);
 				dataSet.getDocuments().add(doc);
 			}
