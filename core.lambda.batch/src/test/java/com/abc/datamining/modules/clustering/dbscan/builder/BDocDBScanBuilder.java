...
//			reduceDimensionsByCHI(documentSet);
	...
	public void reduceDimensionsByCHI(DocumentSet documentSet) {
		...
		for (Document document : documents) {
			...
			int len = list.size() < CHI_WORD_LIMIT ? list.size()
					: CHI_WORD_LIMIT;
	...
	// 获取当前点的邻居
	public List<DataPoint> obtainNeighbors(DataPoint current,
		...
		while (flag) {
			for (DataPoint point : points) {
				...
				if (neighbors.size() >= MIN_POINTS) {
					// 满足核心对象条件的点创建一个新簇
					clusterId = point.getClusterId() <= 0 ? (++clusterId)
							: point.getClusterId();
					System.out
							.println("--------------------------------clusterId: "
									+ clusterId);
	...
	public void print(List<DataPoint> points) {
		Collections.sort(points, new Comparator<DataPoint>() {
			...
			public int compare(DataPoint o1, DataPoint o2) {
				return Integer.valueOf(o1.getClusterId()).compareTo(
						o2.getClusterId());
			}
		});
		for (DataPoint point : points) {
			System.out.println(point.getClusterId() + " - "
					+ point.getCategory());
		}
	}
