package com.abc.datamining.modules.algorithm.lda.sample;
...
public class ComUtil {
	...
	public static void printHash(HashMap<String, Integer> hashMap) {
		...
		while (it.hasNext()) {
			Map.Entry<String, Integer> m = (Map.Entry<String, Integer>) it
					.next();
	...
	public static String getKeysFromValue(HashMap<Integer, String> hm,
		...
		while (it.hasNext()) {
			...
			Map.Entry<Integer, String> m = (Map.Entry<Integer, String>) it
					.next();
	...
	public static double readHashTopValue(HashMap<String, Integer> scores, int k) {
		...
		int value = 0;
		double res = 0;
		for (Iterator<Map.Entry<String, Integer>> it = list.iterator(); count < k
				&& it.hasNext();) {
	...
	public static void getTopNZ(float[] array, int[] counts,
			ArrayList<Integer> rankList, ArrayList<Float> rankProbs, int i,
			int threshold) {
		// clear
		rankList.clear();
		rankProbs.clear();
